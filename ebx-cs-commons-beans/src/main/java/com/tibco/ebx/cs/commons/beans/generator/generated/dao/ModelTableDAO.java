...
/**
 * Data access object (DAO) automatically generated by service 'Generate Java
 * Accessers' of EBX Commons It aims to access records of table at path
 * /root/Table in data model
...
public class ModelTableDAO extends TableDAO<Table> {
	...
	@Override
	protected void setValuesForUpdate(final ValueContextForUpdate pContext, final Table pTable)
			throws EBXCommonsException {
		pContext.setValue(pTable.getIdentifier(), path_to_field_identifier);
		if (pTable.getDataModel() != null) {
			pContext.setValue(DataModelDAO.getInstance().getRecordPrimaryKey(pTable.getDataModel()),
					path_to_field_dataModel);
		}
		pContext.setValue(pTable.getPathInSchema(), path_to_field_pathInSchema);
		if (pTable.getDaobject() != null) {
			pContext.setValue(DataAccessObjectDAO.getInstance().getRecordPrimaryKey(pTable.getDaobject()),
					path_to_field_dao);
		}
		if (pTable.doesInherit(path_to_field_javaBean)) {
			...
			if (pTable.getJavaBean() != null) {
				pContext.setValue(JavaBeanDAO.getInstance().getRecordPrimaryKey(pTable.getJavaBean()),
						path_to_field_javaBean);
			}
		}
		if (pTable.getPreferredDTO() != null) {
			pContext.setValue(DataTransferObjectDAO.getInstance().getRecordPrimaryKey(pTable.getPreferredDTO()),
					path_to_field_preferredDTO);
		}
	}

	@Override
	protected void getValuesFromAdaptation(final Table pTable, final Adaptation pRecord,
			final Optional<SessionPermissions> pPermissions) {
		if (!pPermissions.isPresent() || !pPermissions.get()
				.getNodeAccessPermission(pRecord.getSchemaNode().getNode(path_to_field_identifier), pRecord)
				.isHidden()) {
			pTable.setIdentifier((Integer) pRecord.get(path_to_field_identifier));
		}
		if (!pPermissions.isPresent() || !pPermissions.get()
				.getNodeAccessPermission(pRecord.getSchemaNode().getNode(path_to_field_pathInSchema), pRecord)
				.isHidden()) {
	...
	@Override
	protected void getValuesFromValueContext(final Table pTable, final ValueContext pContext,
			Optional<SessionPermissions> pPermissions) {
		...
		if (!pPermissions.isPresent()
				|| record.isPresent() && !pPermissions.get()
						.getNodeAccessPermission(record.get().getSchemaNode().getNode(path_to_field_identifier),
								record.get())
						.isHidden()
				|| !pPermissions.get()
						.getNodeAccessPermission(pContext.getNode(pathToRecordRoot.add(path_to_field_identifier)),
								pContext.getAdaptationInstance())
						.isHidden()) {
			pTable.setIdentifier((Integer) pContext.getValue(pathToRecordRoot.add(path_to_field_identifier)));
		}
		if (!pPermissions.isPresent()
				|| record.isPresent() && !pPermissions.get()
						.getNodeAccessPermission(record.get().getSchemaNode().getNode(path_to_field_pathInSchema),
								record.get())
						.isHidden()
				|| !pPermissions.get()
						.getNodeAccessPermission(pContext.getNode(pathToRecordRoot.add(path_to_field_pathInSchema)),
								pContext.getAdaptationInstance())
						.isHidden()) {
