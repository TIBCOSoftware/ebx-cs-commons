...
import com.tibco.ebx.cs.commons.lib.utils.FieldsCollector;
...
public class FieldsEnumeration implements ConstraintEnumeration<String> {
	...
	/**
	 * The category or categories (semicolon separated) to consider. All fields are considered if no category is specified
	 */
	 ...
	 * Complete the map.
	 *
	 * @param pNode The node from where fields must be enumerated.
	 */
	...
	public List<String> getValues(final ValueContext pContext) throws InvalidSchemaException {
		if (this.dataSpace != null || this.dataSet != null) {
			Adaptation instance;
			try {
				instance = AdaptationUtils.getDataset(pContext, Optional.ofNullable(this.dataSpace), Optional.ofNullable(this.dataSet));
	 ...
	 * Sets the table path.
	 *
	 * @param tablePath the new table path
	 */
