...
import com.tibco.ebx.cs.commons.ui.ajax.SetValueAjaxComponent;
 ...
 * 
 *
 * This editor aims to set the value of a field according to the current value.
 *
 * It is designed to be used on an foreign key to set a simple string, integer or boolean <br>
 * at location pathToTarget with value at location pathToSource (in record referenced by the FK).
 *
 * <pre>
 * {@code
...
public class SetValueEditor extends UIBeanEditor {
	 ...
	 */
	private void addJsFunctions(final UIResponseContext pContext, final String pOnChangeFunctionName) {
	 ...
	 * @param pOnChangeFunctionName the on change function name
	 */
	private void addOnChangeJsFunction(final UIResponseContext pContext, final String pOnChangeFunctionName) {
		...
		url += "&" + SetValueAjaxComponent.NODE_ID_PARAM + "=" + UIUtils.getUniqueWebIdentifierForCurrentNode(pContext);
