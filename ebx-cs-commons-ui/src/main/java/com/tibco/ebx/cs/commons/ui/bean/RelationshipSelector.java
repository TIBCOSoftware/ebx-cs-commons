...
import com.orchestranetworks.schema.relationships.ExplicitRelationship;
import com.orchestranetworks.schema.relationships.ReverseRelationship;
import com.orchestranetworks.schema.relationships.ReverseRelationshipInterDataset;
import com.orchestranetworks.schema.relationships.ReverseRelationshipIntraDataset;
import com.orchestranetworks.schema.relationships.SchemaNodeRelationships;
...
public class RelationshipSelector extends UIBeanEditor {
	...
	public void addForEdit(final UIResponseContext pContext) {
		...
		Iterable<ExplicitRelationship> explicitRelationships = relationships.getExplicitRelationships();
		...
		Iterable<ReverseRelationship> reverseRelationships = relationships.getReverseRelationships();
		for (ReverseRelationship reverseRelationship : reverseRelationships) {
			if (!reverseRelationship.getExplicitRelationship().getSchemaNodeExplicitRelationship().isTableRefLink()) {
				continue;
			}
			reverseFKs.add(reverseRelationship);
		}

		Iterable<ReverseRelationshipIntraDataset> intraDatasetReverseRelationships = relationships.getIntraDatasetReverseRelationships();
		for (ReverseRelationshipIntraDataset reverseRelationshipIntraDataset : intraDatasetReverseRelationships) {
			if (!reverseRelationshipIntraDataset.getExplicitRelationship().getSchemaNodeExplicitRelationship().isTableRefLink()) {
				continue;
			}
			intraDatasetReverseFKs.add(reverseRelationshipIntraDataset);
		}

		Iterable<ReverseRelationshipInterDataset> interDatasetReverseRelationships = relationships.getInterDatasetReverseRelationships();
		for (ReverseRelationshipInterDataset reverseRelationshipInterDataset : interDatasetReverseRelationships) {
			if (!reverseRelationshipInterDataset.getExplicitRelationship().getSchemaNodeExplicitRelationship().isTableRefLink()) {
				continue;
			}
		...
		pContext.add("<div id='ts_join_filter_" + pContext.getWebName() + "' style='display:none;'>");
	...
	private void addModeSelector(final UIResponseContext pContext) {
		pContext.add("<div id='rs_mode_" + pContext.getWebName() + "' style='padding-bottom:5px;'>");

		String name = "mode_" + pContext.getWebName();
		String onChangeFnName = "refreshRelationshipSelector_" + pContext.getWebName() + "(this)";

		this.addRadioButton(pContext, "direct_" + pContext.getWebName(), name, onChangeFnName, "Direct");
		this.addRadioButton(pContext, "indirect_" + pContext.getWebName(), name, onChangeFnName, "Indirect");
		this.addRadioButton(pContext, "join_" + pContext.getWebName(), name, onChangeFnName, "Over a join table");

		pContext.add("</div>");

		if (!StringUtils.isBlank((String) pContext.getValue(Path.parse("./joinForeignKey")))) {
			pContext.addJS_cr("document.getElementById('join_" + pContext.getWebName() + "').click();");
		} else if (!StringUtils.isBlank((String) pContext.getValue(Path.parse("./table")))) {
			pContext.addJS_cr("document.getElementById('indirect_" + pContext.getWebName() + "').click();");
		} else {
			pContext.addJS_cr("document.getElementById('direct_" + pContext.getWebName() + "').click();");
		}

		pContext.addJS_cr("function refreshRelationshipSelector_" + pContext.getWebName() + "(radio){");
		pContext.addJS_cr("if(radio.checked){");
		pContext.addJS_cr("switch(radio.value){");
		pContext.addJS_cr("case 'direct_" + pContext.getWebName() + "' :");
		pContext.addJS_cr("document.getElementById('rs_table_" + pContext.getWebName() + "').style.display='none';");
		pContext.addJS_cr("document.getElementById('ts_join_fk_" + pContext.getWebName() + "').style.display='none';");
		pContext.addJS_cr("document.getElementById('ts_join_filter_" + pContext.getWebName() + "').style.display='none';");
		pContext.addJS_cr("break;");
		pContext.addJS_cr("case 'indirect_" + pContext.getWebName() + "' :");
		pContext.addJS_cr("document.getElementById('rs_table_" + pContext.getWebName() + "').style.display='';");
		pContext.addJS_cr("document.getElementById('ts_join_fk_" + pContext.getWebName() + "').style.display='none';");
		pContext.addJS_cr("document.getElementById('ts_join_filter_" + pContext.getWebName() + "').style.display='none';");
		pContext.addJS_cr("break;");
		pContext.addJS_cr("case 'join_" + pContext.getWebName() + "' :");
		pContext.addJS_cr("document.getElementById('rs_table_" + pContext.getWebName() + "').style.display='';");
		pContext.addJS_cr("document.getElementById('ts_join_fk_" + pContext.getWebName() + "').style.display='';");
		pContext.addJS_cr("document.getElementById('ts_join_filter_" + pContext.getWebName() + "').style.display='';");
		pContext.addJS_cr("break;");
		pContext.addJS_cr("}");
		pContext.addJS_cr("}");
		pContext.addJS_cr("}");
	...
	private void addRadioButton(final UIResponseContext pContext, final String pID, final String pName, final String pOnChangeFnName, final String pLabel) {
		pContext.add("<input type='radio' id='" + pID + "' name='" + pName + "' value='" + pID + "' onchange='" + pOnChangeFnName + "'/>");
	...
	private void addTableSelector(final UIResponseContext pContext) {
		...
		pContext.add("<div id='rs_select_" + pContext.getWebName() + "' style='padding-bottom:5px;'>");

		String name = "select_" + pContext.getWebName();
		String onChangeFnName = "refreshTableSelector_" + pContext.getWebName() + "(this)";

		this.addRadioButton(pContext, "intra_dataset_" + pContext.getWebName(), name, onChangeFnName, "Same data set");
		this.addRadioButton(pContext, "intra_home_" + pContext.getWebName(), name, onChangeFnName, "Other data set");
		this.addRadioButton(pContext, "inter_home_" + pContext.getWebName(), name, onChangeFnName, "Other data space");
		...
		if (!StringUtils.isBlank((String) pContext.getValue(Path.parse("./dataspace")))) {
			pContext.addJS_cr("document.getElementById('inter_home_" + pContext.getWebName() + "').click();");
		} else if (!StringUtils.isBlank((String) pContext.getValue(Path.parse("./dataset")))) {
			pContext.addJS_cr("document.getElementById('intra_home_" + pContext.getWebName() + "').click();");
		} else {
			pContext.addJS_cr("document.getElementById('intra_dataset_" + pContext.getWebName() + "').click();");
		}
		...
		pContext.addJS_cr("document.getElementById('ts_dataspace_" + pContext.getWebName() + "').style.display='none';");
		pContext.addJS_cr("document.getElementById('ts_dataset_" + pContext.getWebName() + "').style.display='none';");
		pContext.addJS_cr("document.getElementById('ts_table_same_dataset_" + pContext.getWebName() + "').style.display='';");
		pContext.addJS_cr("document.getElementById('ts_table_" + pContext.getWebName() + "').style.display='none';");
		pContext.addJS_cr("break;");
		pContext.addJS_cr("case 'intra_home_" + pContext.getWebName() + "' :");
		pContext.addJS_cr("document.getElementById('ts_dataspace_" + pContext.getWebName() + "').style.display='';");
		pContext.addJS_cr("document.getElementById('ts_dataset_" + pContext.getWebName() + "').style.display='';");
		pContext.addJS_cr("document.getElementById('ts_table_same_dataset_" + pContext.getWebName() + "').style.display='none';");
		pContext.addJS_cr("document.getElementById('ts_table_" + pContext.getWebName() + "').style.display='';");
		pContext.addJS_cr("break;");
	...
	public void validateInput(final UIRequestContext pContext) {
		...
		String select = pContext.getOptionalRequestParameterValue("select_" + pContext.getWebName());
